...
B = imtransform(myimage,T,R,'FillValues',red); 
h2 = figure; imshow(B);
title('Sheared Image');
...
YData = [-99 2100];
R = makeresampler({'cubic','nearest'},'fill');
Bf = imtransform(myimage,T,R,'XData',XData,'YData',YData,...
                 'FillValues',red);
figure, imshow(Bf);
title('Pad Method = ''fill''');

R = makeresampler({'cubic','nearest'},'replicate');
Br = imtransform(myimage,T,R,'XData',XData,'YData',YData);
figure, imshow(Br);
title('Pad Method = ''replicate''');

R = makeresampler({'cubic','nearest'}, 'bound');
Bb = imtransform(myimage,T,R,'XData',XData,'YData',YData,...
                 'FillValues',red);
figure, imshow(Bb);
...
Bc = imtransform(myimage,Thalf,R,'XData',XData,'YData',YData,...
                 'FillValues',red);
figure, imshow(Bc);
title('Pad Method = ''circular''');

R = makeresampler({'cubic','nearest'},'symmetric');
Bs = imtransform(myimage,Thalf,R,'XData',XData,'YData',YData,...
                 'FillValues',red);
figure, imshow(Bs);
